...
def train_dataset(batch_size=32, buffer_size=50000, train_epochs=40):
    """
    tf.data.Dataset object for MNIST training data.
    original data has been saved to DATA_DIRECTORY
    image has been normalized from [0, 255] to [0.0, 1.0]
    dataset has also been shuffled
    """
    ds = dataset(DATA_DIRECTORY, 'train-images-idx3-ubyte', 'train-labels-idx1-ubyte')
    return ds.shuffle(buffer_size=buffer_size).batch(batch_size=batch_size).cache().repeat(train_epochs)


def test_dataset(batch_size=32):
    """
    tf.data.Dataset object for MNIST test data.
    original data has been saved to DATA_DIRECTORY
    image has been normalized from [0, 255] to [0.0, 1.0]
    """
    ds = dataset(DATA_DIRECTORY, 't10k-images-idx3-ubyte', 't10k-labels-idx1-ubyte')
