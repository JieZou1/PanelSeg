...
def default_l_classification_model(
    ...
    # reshape output and apply sigmoid
    if keras.backend.image_data_format() == 'channels_first':
        outputs = keras.layers.Permute((2, 3, 1), name='pyramid_l_classification_permute')(outputs)
...
def default_l_regression_model(num_anchors, pyramid_feature_size=64, regression_feature_size=64, name='l_regression_submodel'):
    ...
    if keras.backend.image_data_format() == 'channels_first':
        outputs = keras.layers.Permute((2, 3, 1), name='pyramid_l_regression_permute')(outputs)
...
def __create_l_pyramid_features(C2, C3, C4, feature_size=64):
    """ Creates the FPN layers on top of the backbone features.
    ...
    return [L_P2, L_P3, L_P4]


class PanelAnchorParameters:
...
"""
PanelAnchorParameters.default = PanelAnchorParameters(
...
The default anchor parameters.
"""
LabelAnchorParameters.default = LabelAnchorParameters(
    sizes   = [16, 32, 64],
    strides = [4, 8, 16],
...
def retinanet(
    ...
    l_features = create_l_pyramid_features(C2, C3, C4)
...
def retinanet_bbox(
    model                 = None,
    anchor_parameters     = PanelAnchorParameters.default,
    label_anchor_parameters = LabelAnchorParameters.default,
    nms                   = True,
    ...
    **kwargs
    ...
    """
    if model is None:
        model = retinanet(num_anchors=anchor_parameters.num_anchors(), l_num_anchors=label_anchor_parameters.num_anchors(), **kwargs)

    # compute the anchors
    features = [model.get_layer(p_name).output for p_name in ['P3', 'P4', 'P5', 'P6', 'P7']]
    anchors  = __build_anchors(anchor_parameters, features)

    l_features = [model.get_layer(p_name).output for p_name in ['L_P2', 'L_P3', 'L_P4']]
    ...
    l_detections = layers.FilterDetections(
        nms                   = nms,
        class_specific_filter = class_specific_filter,
        name                  = 'l_filtered_detections'
    )([l_boxes, l_classification] + other)

    outputs = detections + l_detections
