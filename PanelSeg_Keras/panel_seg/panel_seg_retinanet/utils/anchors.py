...
def anchor_targets_bbox(
    ...
    # l_regression_batch= np.zeros((batch_size, l_anchors.shape[0], 4 + 1), dtype=keras.backend.floatx())
    # l_labels_batch    = np.zeros((batch_size, l_anchors.shape[0], l_num_classes + 1), dtype=keras.backend.floatx())
    # l_annotations_batch = np.zeros((batch_size, l_anchors.shape[0], annotations_group[0].shape[1]), dtype=keras.backend.floatx())

    # compute labels and regression targets
    for index, (image, annotations) in enumerate(zip(image_group, annotations_group)):
        if annotations.shape[0]:
            # obtain indices of gt annotations with the greatest overlap
            positive_indices, ignore_indices, argmax_overlaps_inds = compute_gt_annotations(anchors, annotations[:, :5], negative_overlap, positive_overlap)
